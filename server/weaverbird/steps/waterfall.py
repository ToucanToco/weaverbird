...
class WaterfallStep(BaseStep):
    ...
    def execute(self, df: DataFrame, domain_retriever: DomainRetriever = None,
                execute_pipeline: PipelineExecutor = None) -> DataFrame:
        start_df = df[df[self.milestonesColumn] == self.start].rename(
            columns={self.valueColumn: f'{self.valueColumn}_start'})
        end_df = df[df[self.milestonesColumn] == self.end].rename(columns={self.valueColumn: f'{self.valueColumn}_end'})
        merged_df = start_df.merge(end_df, on=self.labelsColumn)
        merged_df['result'] = merged_df[f'{self.valueColumn}_end'] - merged_df[f'{self.valueColumn}_start']
        result_df = DataFrame(
            {
        ...
        result_df['TYPE_waterfall'] = numpy.where(result_df['LABEL_waterfall'].isin([self.start, self.end]), None, 'Parent')
        result_df[self.valueColumn] = 0
        return merged_df
