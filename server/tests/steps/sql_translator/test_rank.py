...
def test_translate_no_new_column_rank(query):
    ...
    assert (
        ...
        == "WITH SELECT_STEP_0 AS (SELECT * FROM products), RANK_STEP_1 AS (SELECT * FROM TOTO, RAICHU, FLORIZARRE, "
        "(RANK() OVER (ORDER BY RAICHU asc)) AS RAICHU_RANK)"
    )
...
def test_translate_rank(query):
    ...
    assert (
        query.transformed_query
        == "WITH SELECT_STEP_0 AS (SELECT * FROM products), RANK_STEP_1 AS (SELECT * FROM TOTO, RAICHU, FLORIZARRE, "
        "(RANK() OVER (ORDER BY RAICHU asc)) AS DOUMBA)"
    )
