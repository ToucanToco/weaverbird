...
def test_filter(pipeline_translator):
    query, _ = pipeline_translator(
        Pipeline(
            steps=[
                {'name': 'domain', 'domain': 'domain_a'},
                {
                    'name': 'filter',
                    'condition': {'column': 'title', 'operator': 'eq', 'value': 'tutu'},
                },
            ]
        )
    )
    assert query == 'select * from (select title from books) where title = tutu'
...
def test_report(pipeline_translator):
    ...
    assert len(report.steps_translation_reports) == 2
