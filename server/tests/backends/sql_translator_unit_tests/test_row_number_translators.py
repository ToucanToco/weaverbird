...
def get_top_query(sort_order, previous_step, group, rank_on, selected_columns, limit):
    ...
    rank_select = sub_query.select(
        RowNumber().as_("row_number").over(name_field).orderby(age_field, order=getattr(Order, sort_order))
    )
    expected_query = (
...
def test_top_with_enabled_row_number(
    ...
    ctx = row_number_enabled_translator.top(step=step, columns=selected_columns, **default_step_kwargs)
...
def test_top_with_disabled_row_number(
    ...
    with pytest.raises(NotImplementedError):
        row_number_disabled_translator.top(step=step, columns=selected_columns, **default_step_kwargs)
...
def test_argmax_with_enabled_split_part(
    ...
    ctx = row_number_enabled_translator.argmax(step=step, columns=selected_columns, **default_step_kwargs)
...
def test_argmax_with_disabled_split_part(
    ...
    step = steps.ArgmaxStep(column=rank_on, groups=[group])
    with pytest.raises(NotImplementedError):
        row_number_disabled_translator.argmax(step=step, columns=selected_columns, **default_step_kwargs)
...
def test_argmin_with_enabled_split_part(
    ...
    ctx = row_number_enabled_translator.argmin(step=step, columns=selected_columns, **default_step_kwargs)
...
def test_argmin_with_disabled_split_part(
    ...
    with pytest.raises(NotImplementedError):
        row_number_disabled_translator.argmin(step=step, columns=selected_columns, **default_step_kwargs)
