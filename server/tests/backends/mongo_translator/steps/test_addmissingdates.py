...
def test_add_missing_date_generation():
    assert translate_addmissingdates(
        AddMissingDatesStep(
            name="addmissingdates", dates_column="Year", dates_granularity="month", groups=[]
        )
    ) == [
        {
            "$addFields": {
                "_vqbDay": {
                    "$dateFromParts": {"year": {"$year": "$Year"}, "month": {"$month": "$Year"}}
                }
            }
        },
        {
        ...
        },
        {
            "$addFields": {
                "_vqbMinMaxDiffInDays": {
                    "$divide": [{"$subtract": ["$_vqbMaxDay", "$_vqbMinDay"]}, 86400000]
                }
            }
        },
        {
                        ...
                        "vars": {
                            "currentColumns": {
                                "$arrayToObject": {
                                    "$map": {
                                        "input": {
                                            "$objectToArray": {"$arrayElemAt": ["$_vqbArray", 0]}
                                        },
                                        "as": "field",
                                        "in": {"k": "$$field.k", "v": None},
                                    }
                            ...
                            "$map": {
                                ...
                                "in": {
                                    "$let": {
                                        "vars": {
                                            "dateIndex": {
                                                "$indexOfArray": ["$_vqbArray._vqbDay", "$$date"]
                                            }
                                        },
                                        "in": {
                                            "$cond": [
