...
export class Mongo36Translator extends BaseTranslator {
  ...
  transformAppend(step: Readonly<S.AppendStep>): MongoStep[] {
    const facet: { [K: string]: MongoStep[] } = {};
    for (let i = 0; i < step.pipelines.length; i++) {
      facet[`_vqPipeline${i}`] = this.translate(step.pipelines[i]);
    }
    return [
      { $facet: facet },
      {
        $project: {
          _vqbRoot: {
            $concatArrays: Object.keys(facet).map(p => $$(p)),
          },
        },
      },
      { $unwind: '$_vqbRoot' },
      { $replaceRoot: { newRoot: '$_vqbRoot' } },
      { $project: { _id: 0 } },
    ];
  }
}
